...
class Spectrum:
    ...
    def cal_dis_mat(self, data):
        ...
        for i in range(self.N):
            for j in range(i + 1, self.N):
                dis_mat[i, j] = np.sqrt(np.dot((data[i] - data[j]) , (data[i] - data[j])))
    ...
    def fit(self, data):
        ...
        if self.method == 'unnormalized':
            ...
            sqrtDegreeMatrix = np.diag(1.0 / (self.D** (0.5)))
            #L = D * self.L * D
            L=sqrtDegreeMatrix * self.L * sqrtDegreeMatrix 
            w, v = np.linalg.eig(L)
            inds = np.argsort(w)[:self.n_cluster]
            Vectors = v[:, inds]
